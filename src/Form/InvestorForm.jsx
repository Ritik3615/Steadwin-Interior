...
export default function InvestorForm({ endpoint, onSuccess }) {
  ...
  const validate = () => {
    ...
    if (!form.consent)
      e.consent = "We need your consent to contact you about investor relations.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (endpoint) {
        ...
        setStatus("success");
        setStatusMessage("Mail client opened. If it didn't, please email investors@steadwingroup.com");
      }
    } catch (err) {
  ...
  return (
    ...
    <section className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6 md:p-10 mt-32">
      ...
      <p className="text-gray-700 mb-6">
        We invite investors who believe in innovation, quality, and sustainable
        business models. Partner with us and be part of a transformative vision.
      </p>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="block md:col-span-2">
            ...
            <input
              ...
              className={`mt-1 block w-full rounded-lg border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#2b5d7c] ${
                errors.expectedInvestment ? "border-red-300" : "border-gray-200"
              }`}
            />
        ...
        <div className="mt-6 flex items-center gap-4">
          <button
            type="submit"
            disabled={submitting}
            className={`inline-flex items-center justify-center px-6 py-2 rounded-full text-white font-semibold shadow ${
              submitting ? "bg-gray-400 cursor-not-allowed" : "bg-[#2b5d7c] hover:bg-[#1f445c]"
            }`}
          >
            {submitting ? (
              <svg
          ...
          <div className="text-sm text-gray-500 ml-auto">
            <div>Or email: <a href="mailto:investors@steadwingroup.com" className="underline">investors@steadwingroup.com</a></div>
          </div>
        </div>
      </form>
    </section>
